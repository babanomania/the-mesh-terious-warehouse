...
REGION = "south"
rng = BaseGenerator(f"order_error_{REGION}")


class OrderErrorEvent(BaseModel):
    event_id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    event_ts: str = Field(default_factory=lambda: datetime.utcnow().isoformat())
    event_type: str = "order_error"
    error_id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    order_id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    error_code: str
    detected_ts: str = Field(default_factory=lambda: datetime.utcnow().isoformat())


ERROR_CODES = ["OUT_OF_STOCK", "PAYMENT_FAILED", "ADDRESS_INVALID", "UNKNOWN"]


def generate_event() -> OrderErrorEvent:
    error_code = rng.choice(ERROR_CODES)
    return OrderErrorEvent(error_code=error_code)


def publish_events(
    channel: pika.adapters.blocking_connection.BlockingChannel,
    queue: str,
    events: Iterable[OrderErrorEvent],
) -> None:
    for event in events:
        try:
            payload = event.json()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Produce order error events for the south region")
