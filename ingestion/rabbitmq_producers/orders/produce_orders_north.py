...
def publish_events(channel: pika.adapters.blocking_connection.BlockingChannel, queue: str, events: Iterable[OrderEvent]) -> None:
    for event in events:
        try:
            payload = event.json()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Produce order events for the north region")
