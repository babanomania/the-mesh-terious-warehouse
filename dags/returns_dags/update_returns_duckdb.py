"""Create DuckDB views for returns module once (idempotent)."""
...
def create_views() -> None:
    ...
    con.execute("CREATE SCHEMA IF NOT EXISTS returns;")
    wh = os.getenv("ICEBERG_WAREHOUSE", "s3://warehouse").rstrip('/')
    q = (
        "CREATE OR REPLACE VIEW returns.fact_returns AS "
        + "select * from iceberg_scan('"
        + wh
        + "/returns/fact_returns', allow_moved_paths = true)"
    )
    try:
        con.execute(
            "select 1 from iceberg_scan('" + wh + "/returns/fact_returns', allow_moved_paths = true) limit 0"
        )
        con.execute(q)
    except Exception as exc:
        logging.warning("returns.fact_returns not present; skipping view creation: %s", exc)


with DAG(
    dag_id="update_returns_duckdb",
    schedule=None,
    start_date=days_ago(1),
    catchup=False,
    tags=["bootstrap", "duckdb", "returns"],
