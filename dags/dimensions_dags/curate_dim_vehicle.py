"""Curate dimensions.dim_vehicle incrementally into Iceberg."""
from __future__ import annotations

import datetime
from datetime import timedelta

from airflow import DAG
from airflow.providers.standard.operators.python import PythonOperator
from airflow.utils import timezone

from utils.iceberg_duckdb import duckdb_attach_iceberg, max_ts, with_ts_filter, append_arrow_to_iceberg, table_expr


def days_ago(n: int) -> datetime.datetime:
    return timezone.utcnow() - datetime.timedelta(days=n)


DEFAULT_ARGS = {"owner": "data-eng", "retries": 0, "sla": timedelta(minutes=15)}


def curate() -> None:
    import duckdb

    con = duckdb.connect(database=":memory:")
    duckdb_attach_iceberg(con)

    m = max_ts(con, "dimensions", "dim_vehicle", "event_date")
    sql = with_ts_filter(
        """
        with src as (
          select vehicle_id, cast(event_ts as date) as event_date
          from {raw}
          where vehicle_id is not null and vehicle_id <> ''
        )
        select vehicle_id,
               cast(null as varchar) as type,
               cast(null as int) as capacity,
               cast(null as varchar) as current_location,
               event_date
        from src
        """,
        "event_date",
        m,
    )
    at = con.execute(sql.format(raw=table_expr("dispatch_logs", "raw_dispatch_logs"))).fetch_arrow_table()

    append_arrow_to_iceberg(
        "dimensions.dim_vehicle",
        [
            {"name": "vehicle_id", "dataType": "STRING"},
            {"name": "type", "dataType": "STRING"},
            {"name": "capacity", "dataType": "INT"},
            {"name": "current_location", "dataType": "STRING"},
            {"name": "event_date", "dataType": "DATE"},
        ],
        at,
    )


with DAG(
    dag_id="curate_dim_vehicle",
    schedule="@daily",
    start_date=days_ago(1),
    catchup=False,
    tags=["dimensions", "dim_vehicle", "iceberg"],
    default_args=DEFAULT_ARGS,
) as dag:
    PythonOperator(task_id="curate_dim_vehicle", python_callable=curate)
