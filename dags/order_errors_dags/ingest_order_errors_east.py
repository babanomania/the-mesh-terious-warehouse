"""Ingest order error events from RabbitMQ to Iceberg for east region."""
...
class OrderErrorEvent(BaseModel):
    ...
    order_id: str
    error_code: str
    # Producers emit detected_ts; map it to error_ts in the raw table
...
with DAG(
    dag_id="ingest_order_errors_east",
    schedule="@hourly",
    start_date=days_ago(1),
    catchup=False,
    tags=["ingest", "order_errors", "east"],
    default_args={"owner": "data-eng", "retries": 1},
) as dag:
    build_ingest_operator(
        dag_id="ingest_order_errors_east",
        queue_name="errors_east",
...
logger.info("Configured ingest_order_errors_east DAG")
