"""Airflow DAG to update OpenMetadata descriptions for dispatch_logs datasets.

This DAG updates the descriptions for:
- dispatch_logs.raw_dispatch_logs (raw table)
- dispatch_logs.stg_dispatch_logs (staging view/table)
- dispatch_logs.fact_dispatch_logs (fact view/table)
...
def _ensure_lineage(
    ...
    db_name: str = "warehouse",
    upstream_schema: str = "dispatch_logs",
    downstream_schema: str = "dispatch_logs",
) -> None:
...
with DAG(
    dag_id="update_dispatch_logs_metadata",
    schedule="@daily",
    start_date=days_ago(1),
    catchup=False,
    tags=["metadata", "dispatch_logs"],
    ...
    RAW_COLUMNS = [
        ...
        {"name": "dispatch_id", "dataType": "STRING", "description": "Dispatch identifier."},
        {"name": "order_id", "dataType": "STRING", "description": "Associated order identifier."},
        {"name": "vehicle_id", "dataType": "STRING", "description": "Assigned vehicle identifier."},
        {"name": "status", "dataType": "STRING", "description": "Dispatch status."},
    ...
    update_raw = PythonOperator(
        task_id="update_raw_dispatch_logs_metadata",
        python_callable=_update_table_metadata,
        op_kwargs={
            "table_name": "raw_dispatch_logs",
            "description": "Raw dispatch logs ingested from RabbitMQ.",
            "columns": RAW_COLUMNS,
