"""Create DuckDB views for dispatch_logs module once (idempotent)."""
...
def create_views() -> None:
    ...
    con.execute("CREATE SCHEMA IF NOT EXISTS dispatch_logs;")
    wh = os.getenv("ICEBERG_WAREHOUSE", "s3://warehouse").rstrip('/')
    q = (
        "CREATE OR REPLACE VIEW dispatch_logs.fact_dispatch_logs AS "
        + "select * from iceberg_scan('"
        + wh
        + "/dispatch_logs/fact_dispatch_logs', allow_moved_paths = true)"
    )
    try:
        con.execute(
            "select 1 from iceberg_scan('" + wh + "/dispatch_logs/fact_dispatch_logs', allow_moved_paths = true) limit 0"
        )
        con.execute(q)
    except Exception as exc:
        logging.warning(
            "dispatch_logs.fact_dispatch_logs not present; skipping view creation: %s", exc
        )


with DAG(
    dag_id="update_dispatch_logs_duckdb",
    schedule=None,
    start_date=days_ago(1),
    catchup=False,
    tags=["bootstrap", "duckdb", "dispatch_logs"],
